...
// 기존의 모든 함수 정의가 여기에 와야 합니다.
// (생략된 부분)
// ... handleResponse, handleError, increaseFont, decreaseFont, applyFontSize ...
// ... getAutoPlayParams, startAutoPlay, playNextItem, scheduleNextOrRepeat, moveToNextWord ...
// ... scheduleNextPlay, pauseAutoPlay, resumeAutoPlay, stopAutoPlay ... (updateAutoPlayButtons는 위에서 수정됨)
// ... highlightTableRowByIndex, unhighlightAllTableRows ...
// ... generateLanguageMenu, updateMenuSelectionVisuals, setWordLanguage, setMeaningLanguage ...
// ... selectExamTypeAndStart, getFilterDisplayName, resetExamUI, startExamWithRange, getExamTypeName ...
// ... startExam (새로운 파라미터 버전), updateExamProgress, displayNextMCQuestion ...
// ... handleMCAnswerClick, selectMCAnswer, checkMCAnswer ...
// ... setupMatchingGame, shuffleArray, populateMatchingColumn, handleMatchItemClick, checkMatch ...
// ... nextExamItem, showExamResults, exitExam ...
// ... sortFolders, setupAccordionMenus (중요: 아코디언 메뉴 JS는 새 구조에서도 잘 동작해야 함) ...
// ... backupVocabulary, triggerBackupLoad, handleBackupFileLoad, escapeCsvField, exportVocabulary ...
// ... setupColumnResizing, removeResizeListeners, createResizeListeners, initResize, handleMove, handleEnd ...
// ... addMoveEndListeners, removeMoveEndListeners ...
// ... enterSelectionMode, cancelSelectionMode, toggleSelectAll, deleteSelectedWords, getSelectedWordIndices ...
// ... triggerMoveSelectedWords, moveSelectedWords, showFolderSelectModal, closeFolderSelectModal ...
// ... setupContextMenuListeners, handleItemContextMenu, handleItemTouchStart, handleItemTouchMove, handleItemTouchEnd ...
// ... hideContextMenu, showContextMenu, handleContextMenuMoveToOther, handleContextMenuToggleSelectionMode ...
// ... moveSingleWord, moveMultipleWords, handleContextMenuEdit, handleContextMenuInsertAbove, handleContextMenuDelete ...
// ... handleContextMenuMoveToReview, handleContextMenuRestoreOrigin, insertWordAbove, moveWordToReview, restoreWordFromReview ...
// ... handleFolderContextMenuNewSubfolder, createNewSubfolderUnder, handleFolderEvent_Delegated ...
// ... handleFolderContextMenu_Delegated, handleFolderTouchStart_Delegated, handleFolderTouchMove_Delegated, handleFolderTouchEnd_Delegated ...
// ... showFolderContextMenu, hideFolderContextMenu, handleFolderContextMenuRename, handleFolderContextMenuDelete ...
// ... renameSpecificFolder, deleteSpecificFolder, handleClickOutsideMenus_Combined ...
...
function loadFoldersAndSelectInitial() {
    console.log("[JS] loadFoldersAndSelectInitial started");
    fetch('/api/folders')
        .then(handleResponse)
        .then(d => {
            console.log("[JS] Folders fetched successfully:", d);
            folders = d.folders || [];
            renderFolderTree(folders);
            let path_to_select = null;
            if (window.initialPathToSelect) {
                path_to_select = window.initialPathToSelect;
            } else {
                 const reviewNode = findFolderNode(folders, "복습 절실");
                 if (reviewNode) { path_to_select = getFolderPathRecursive(folders, reviewNode.name); }
                 if (!path_to_select && folders && folders.length > 0) { path_to_select = getFirstValidPath(folders); }
            }
            if (path_to_select) { selectFolder(path_to_select); }
            else { renderWordList(); renderSentenceView(); }
        })
        .catch(e => {
            console.error("[JS][오류] 폴더 목록 가져오기 실패:", e);
            handleError(new Error("폴더 목록을 가져오는 데 실패했습니다."));
            folders = [];
            renderFolderTree([]); renderWordList(); renderSentenceView();
        });
     console.log("[JS] loadFoldersAndSelectInitial finished.");
}
...
function loadFoldersFromServer() { fetch('/api/folders').then(handleResponse).then(d => { folders = d.folders || []; renderFolderTree(folders); }).catch(handleError); }
...
function createFolder() { const n = prompt('새 폴더 이름:'); if (n && n.trim()) { fetch('/api/folder', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ folder_name: n.trim() }), }).then(handleResponse).then(d => { folders = d.folders; renderFolderTree(folders); }).catch(handleError); } }
...
function selectFolder(folderPath) {
    ...
    document.querySelectorAll('#folderSidebar .folder-item').forEach(item => { item.classList.toggle('selected', item.dataset.folderPath === folderPath); });
    expandParentFolders(folderPath);
    if (window.AndroidInterface && typeof window.AndroidInterface.storeLastFolderPath === 'function') { try { window.AndroidInterface.storeLastFolderPath(folderPath); } catch (error) { console.error("Android 저장 오류", error); } }
    if (isCardViewActive) toggleCardView(); 
    if (isExamModeActive) exitExam();       
    if (isSentenceModeActive) {
        toggleSentenceMode(); // 문장 모드였다면 닫고 단어 목록으로 (필터는 유지됨)
    }
    if (isInSelectionMode) cancelSelectionMode(); 

    fetch(`/api/words?path=${encodeURIComponent(folderPath)}`)
        .then(handleResponse)
        .then(d => {
            currentWords = d.words || []; 
            wordHidden = {}; 
            meaningHidden = {};
            // selectFolder 후에는 항상 단어 목록을 먼저 보여주거나,
            // 이전 뷰 상태를 기억했다가 복원할 수 있음.
            // 현재는 단어 목록을 기본으로 표시.
            if(isSentenceModeActive) isSentenceModeActive = false; // 문장 모드 해제
            document.body.classList.remove('sentence-mode-active'); // CSS 클래스도 제거
            renderWordList(); 
            
            const sidebar = document.getElementById('folderSidebar');
            if (sidebar && sidebar.classList.contains('open')) { toggleSidebar(); }
        })
        .catch(e => { handleError(e); currentWords = []; renderWordList(); renderSentenceView(); });
}
...
function playTTS(text, languageCode, voiceName, onEndCallback) { if (!text || text.trim() === '') { if (onEndCallback) try { onEndCallback(); } catch (e) { console.error("onEndCallback error:", e); } return; } if (ttsAudio && !ttsAudio.paused) { ttsAudio.pause(); ttsAudio.onended = null; ttsAudio.onerror = null; if (ttsAudio.src && ttsAudio.src.startsWith('blob:')) { try { URL.revokeObjectURL(ttsAudio.src); } catch (e) {} } } ttsAudio = new Audio(); fetch('/api/tts', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: text, language_code: languageCode, voice_name: voiceName }), }).then(r => { const ct = r.headers.get("content-type"); if (!r.ok) { if (ct && ct.includes("application/json")) { return r.json().then(eData => { throw new Error(eData.error || `TTS 요청 실패 (상태: ${r.status})`); }); } else { throw new Error(`TTS 요청 실패 (상태: ${r.status}, 응답 유형: ${ct || '알 수 없음'})`); } } return r.blob(); }).then(blob => { const audioUrl = URL.createObjectURL(blob); if (!ttsAudio) ttsAudio = new Audio(); ttsAudio.src = audioUrl; ttsAudio.onended = () => { try { URL.revokeObjectURL(audioUrl); } catch (e) {} ttsAudio = new Audio(); if (onEndCallback) try { onEndCallback(); } catch (e) { console.error("onEndCallback error:", e); } }; ttsAudio.onerror = (e) => { console.error("TTS 재생 오류:", e); try { URL.revokeObjectURL(audioUrl); } catch (e) {} ttsAudio = new Audio(); if (isAutoPlaying) stopAutoPlay(); if (onEndCallback) try { onEndCallback(); } catch (e) {} }; if (!isAutoPaused) { const pp = ttsAudio.play(); if (pp !== undefined) { pp.catch(error => { console.error("TTS 재생 시작 오류:", error); try { URL.revokeObjectURL(audioUrl); } catch (e) {} ttsAudio = new Audio(); if (isAutoPlaying) stopAutoPlay(); if (onEndCallback) try { onEndCallback(); } catch (e) {} }); } } else { try { URL.revokeObjectURL(audioUrl); } catch (e) {} ttsAudio = new Audio(); if (onEndCallback) try { onEndCallback(); } catch (e) {} } }).catch(e => { handleError(e); if (isAutoPlaying) stopAutoPlay(); if (onEndCallback) try { onEndCallback(); } catch (e) {} }); }
...
async function addWord() {
    if (!currentFolderPath) { alert('추가할 폴더 선택'); return; }
    let c = 0;
    while (true) {
        const w = prompt('단어 (취소 또는 빈 칸 시 종료):');
        if (w === null || w.trim() === '') break;
        const m = prompt(`'${w}' 뜻 (취소 또는 빈 칸 시 종료):`);
        if (m === null || m.trim() === '') break;
        const pos = prompt(`'${w}' 품사 (선택사항, 예: n, v, adj. 취소 시 빈 칸):`, '');
        try {
            const newEntry = {
                word: w.trim(),
                meaning: m.trim(),
                part_of_speech: pos ? pos.trim() : null,
                entries: [],
                isStudied: false, 
            ...
            const r = await fetch(`/api/words`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ path: currentFolderPath, ...newEntry }),
            });
            ...
            handleError(e); alert('오류 발생, 중단'); break;
        }
    }
    if (c > 0) console.log(`${c}개 단어 추가 완료.`);
}
...
function deleteWord(index) {
    if (!currentFolderPath) { alert('삭제할 폴더 선택'); return; }
    if (index === undefined || isNaN(index) || index < 0 || index >= currentWords.length) { alert('유효하지 않음'); return; }
    const entry = currentWords[index];
    if (!entry || typeof entry !== 'object') { alert('삭제할 항목 데이터 오류'); return; }
    const wordToDelete = entry.word || '[알 수 없는 단어]';
    if (confirm(`'${wordToDelete}' 항목 삭제?`)) {
        fetch(`/api/words/${index}`, {
            method: 'DELETE',
            headers: {'Content-Type': 'application/json'},
...
function deleteAllWords() { if (!currentFolderPath) { alert("삭제할 폴더 선택"); return; } const fn = currentFolderPath.split('/').pop(); if (confirm(`'${fn}' 모든 단어/문장 삭제?`)) { fetch(`/api/words`, { method: 'DELETE', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ path: currentFolderPath }) }).then(handleResponse).then(d => { currentWords = d.words; renderWordList(); renderSentenceView(); alert(`'${fn}' 모든 항목 삭제됨.`); }).catch(handleError); } }
...
function backupVocabulary() { fetch('/api/backup_data').then(handleResponse).then(bd=>{ if(!bd||typeof bd!=='object'||Object.keys(bd).length===0){ alert('백업 데이터 없음'); return; } try { const jd=JSON.stringify(bd, null, 2); const ts=new Date().toISOString().replace(/[:.]/g, '-'); const fn=`vocabulary_backup_${ts}.json`; if(window.AndroidInterface&&typeof window.AndroidInterface.saveJsonBackup==='function'){ window.AndroidInterface.saveJsonBackup(jd, fn); } else { const b=new Blob([jd], { type:'application/json' }); const u=URL.createObjectURL(b); const a=document.createElement('a'); a.href=u; a.download=fn; document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(u); } } catch(e){ handleError(new Error(`백업 처리 오류: ${e.message}`)); } }).catch(error=>{ handleError(new Error(`백업 데이터 로드 실패: ${error.message}`)); }); }
function triggerBackupLoad() { const i=document.getElementById('backup-file-input'); if(i)i.click(); }
function handleBackupFileLoad(event) { const f=event.target.files[0]; if(!f)return; if(!f.name.toLowerCase().endsWith('.json')){ alert('JSON 파일(.json) 선택'); event.target.value=null; return; } const r=new FileReader(); r.onload=function(e){ try{ const bd=JSON.parse(e.target.result); if(typeof bd!=='object'||bd===null||Array.isArray(bd)){throw new Error("유효하지 않은 JSON 형식");} fetch('/api/folders/restore_backup', { method:'POST', headers:{ 'Content-Type':'application/json' }, body:JSON.stringify(bd), }).then(handleResponse).then(d=>{ alert('백업 복원 성공'); folders = d.folders || []; currentFolderPath=null; currentWords=[]; renderFolderTree(folders); renderWordList(); renderSentenceView(); if(folders&&folders.length>0){ const fp=getFirstValidPath(folders); if(fp)selectFolder(fp); } }).catch(handleError); } catch(error){ handleError(new Error(`백업 파일 처리 오류: ${error.message}`)); } finally{ event.target.value=null; } }; r.onerror=()=>{ alert("파일 읽기 오류"); event.target.value=null; }; r.readAsText(f); }
